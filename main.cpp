...
#define DEFAULT_COLOR \
    {                 \
        60, 60, 60    \
    }                                               // color used when the sampling ray fails to hit the target
int INITIAL_TIME = 0;                               // the 0 is when the plane moves to the origin
...
spectrum_1step convert_spectrum(Vector3 pos, spectrum_1step &sp)
{
    ...
    double headlight_coeff = (Beta * C * global_time) * (Beta * C * global_time) / (mygamma * Beta * C * global_time - pos.x / mygamma) / (mygamma * Beta * C * global_time - pos.x / mygamma) / mygamma;
    headlight_coeff /= divide;
    for (int i = 0; i < 400; i++)
    {
        double freq_orig = C / (((double)i + 380.0) / 1e9);
        double freq_trans = freq_orig * (Beta * C * global_time - pos.x) / (mygamma * Beta * C * global_time - pos.x / mygamma);
...
{
    Vector3 res = {0, 0, 0};
    res.x = -mygamma * Beta * (C * global_time - hit_pos.mod()) + mygamma * hit_pos.x;
    res.y = hit_pos.y;
    res.z = hit_pos.z;
    return res;
}

void mysample(int st_y, int st_z, int ed_y, int ed_z, int &cnt, int id)
{
    string s = "thread_";
    s += to_string(id);
    s += ".log";
    ...
    for (int i = start_y; i <= end_y; i++) // from -0.5 to 0.5
    {
        ...
        {
            cnt++;
            if ((i + (RES_WIDTH >> 1)) < 0 || ((RES_HEIGHT >> 1) - j) < 0)
                continue;
            ...
            {
                while (tmp2.y < 0)
                    tmp2.y += TILING_FACTOR * grid_size;
                while (tmp2.z < 0)
                    tmp2.z += TILING_FACTOR * grid_size;
                while (tmp2.y > TILING_FACTOR * grid_size)
                    tmp2.y -= TILING_FACTOR * grid_size;
                while (tmp2.z > TILING_FACTOR * grid_size)
                    tmp2.z -= TILING_FACTOR * grid_size;
            ...
            double yp = (original_pos.y + original_pos.z) / TILING_FACTOR;
            double zp = (original_pos.y - original_pos.z) / TILING_FACTOR;
            spectrum_1step temp;
            if (((long long)(floor(yp) + floor(zp)) & 1))
            {
...
int main()
{
    ...
    while (1)
    {
        ...
        mygamma = pow((1.0 - Beta * Beta), -0.5);
        CImg<unsigned char> res_img(RES_WIDTH, RES_HEIGHT, 1, 3, 0);
        thread tlist[100];
        for (int i = 0; i < threads; i++)
        {
            tlist[i * 2] = thread(mysample, (-(RES_WIDTH >> 1) + del_y * i), 0, -(RES_WIDTH >> 1) + del_y * (i + 1) - 1, -(RES_HEIGHT >> 1), ref(cnts[i * 2]), i * 2 + 1);
            tlist[i * 2 + 1] = thread(mysample, (-(RES_WIDTH >> 1) + del_y * i), (RES_WIDTH >> 1), -(RES_WIDTH >> 1) + del_y * (i + 1) - 1, 1, ref(cnts[i * 2 + 1]), i * 2 + 2);
